...
//                    Log.e(TAG, String.valueOf(photoUrl));
           ...
           // Glide.with(this).load(photoUrl).fitCenter().override(120,120).into(ivUserImagProfile);
            ...
            if(providerId.equals("google.com") || providerId.equals("facebook.com")){
                ...
                Glide.with(this).load(R.drawable.img_profile_em).asBitmap().into(new BitmapImageViewTarget(ivUserImagProfile) {
                    @Override
                    protected void setResource(Bitmap resource) {
                        RoundedBitmapDrawable circularBitmapDrawable =
                                RoundedBitmapDrawableFactory.create(getResources(), resource);
            ...
            userReference.orderByKey().equalTo(ID).addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    if (!dataSnapshot.exists()) {
                        user = new User(ID, email, "pass", name, image, 1, "Recluta", 0, 0, 0, 0, 0, 100, 0, 0, 0, 0, 0, 0, 0, 0, false,0);
